...
import { endpointOf, withQuery } from './ows.mjs';
...
export function buildQuery({ text: q, protocols = RENDERABLE_PROTOCOLS, bbox, size = 20, from = 0 } = {}) {
    const must = [];
    if (q) must.push({ query_string: { query: expandTerms(q), default_operator: 'AND' } });
...
/**
 * A typed query, widened to what a person meant.
 *
 * The index stores whole analyzed terms, so a bare query_string matches only
 * complete words: searching a Walloon catalogue for "scolyt" returns nothing
 * while "scolytés" returns three, and searching a German-Swiss catalogue for
 * "forest" finds nine records where "Wald" finds 335. Each term is therefore
 * offered both exact and as a prefix, so a stem or a partially typed word still
 * lands, while the terms are still ANDed together — typing more words should
 * narrow the result, not widen it.
 *
 * Operators the caller wrote themselves are left alone: a query already
 * containing AND, OR, NOT, quotes or wildcards is passed through, because
 * rewriting it would change what they asked for.
 */
...
/**
 * The endpoint of a link, with the request keywords stripped off.
 *
 * Re-exported from lib/ows.mjs so callers of this module keep working. Only the
 * parameters we compose ourselves are removed; a vendor parameter such as
 * MapServer's ?map= is part of the endpoint and stays.
 */
export { endpointOf };
...
export function wmsTileUrl(endpoint, layer, { version = '1.3.0', format = 'image/png' } = {}) {
    ...
    return withQuery(endpoint,
        `LAYERS=${encodeURIComponent(layer)}&SERVICE=WMS&VERSION=${version}`
    ...
    for (const record of records) {
        for (const link of renderableLinks(record, protocols)) {
            ...
            const svc = services.get(key) ?? {
                ...
                type, endpoint,
                capabilitiesUrl: withQuery(endpoint, `SERVICE=${type.toUpperCase()}&REQUEST=GetCapabilities`),
                layers: [],
            };
